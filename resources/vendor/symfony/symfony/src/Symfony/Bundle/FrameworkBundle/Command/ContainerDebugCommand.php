...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('parameters')) {
            $object = Self::$getContainerBuilder()->getParameterBag();
            $options = array();
        } elseif ($parameter = $input->getOption('parameter')) {
            $object = Self::$getContainerBuilder();
            ...
            $object = Self::$getContainerBuilder();
            $name = Self::$findProperServiceName($input, $io, $object, $name);
    ...
    {
        if (Self::$containerBuilder) {
            return Self::$containerBuilder;
        }

        if (!Self::$getApplication()->getKernel()->isDebug()) {
            throw new \LogicException(sprintf('Debug information about the container is only available in debug mode.'));
        }

        if (!is_file($cachedFile = Self::$getContainer()->getParameter('debug.container.dump'))) {
        ...
        return Self::$containerBuilder = $container;
    }
    ...
    private function findProperServiceName(InputInterface $input, SymfonyStyle $io, ContainerBuilder $builder, $name)
    {
        ...
        $matchingServices = Self::$findServiceIdsContaining($builder, $name);
