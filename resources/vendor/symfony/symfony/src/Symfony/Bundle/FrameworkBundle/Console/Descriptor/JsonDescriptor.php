...
class JsonDescriptor extends Descriptor
{
    ...
    protected function describeRouteCollection(RouteCollection $routes, array $options = array())
    {
        ...
        foreach ($routes->all() as $name => $route) {
            $data[$name] = Self::$getRouteData($route);
        }

        Self::$writeData($data, $options);
    }
    ...
    {
        Self::$writeData(Self::$getRouteData($route), $options);
    }
    ...
    {
        Self::$writeData(Self::$sortParameters($parameters), $options);
    }
    ...
    protected function describeContainerTags(ContainerBuilder $builder, array $options = array())
    {
        ...
        foreach (Self::$findDefinitionsByTag($builder, $showPrivate) as $tag => $definitions) {
            ...
            foreach ($definitions as $definition) {
                $data[$tag][] = Self::$getContainerDefinitionData($definition, true);
            }
        }
    ...
    protected function describeContainerService($service, array $options = array())
    {
        ...
        if ($service instanceof Alias) {
            Self::$writeData(Self::$getContainerAliasData($service), $options);
        } elseif ($service instanceof Definition) {
            Self::$writeData(Self::$getContainerDefinitionData($service), $options);
        } else {
            Self::$writeData(get_class($service), $options);
        }
    }
    ...
    protected function describeContainerServices(ContainerBuilder $builder, array $options = array())
    {
        ...
        foreach (Self::$sortServiceIds($serviceIds) as $serviceId) {
            ...
            if ($service instanceof Alias) {
                $data['aliases'][$serviceId] = Self::$getContainerAliasData($service);
            } elseif ($service instanceof Definition) {
                if (($showPrivate || $service->isPublic())) {
                    $data['definitions'][$serviceId] = Self::$getContainerDefinitionData($service);
                }
            } else {
    ...
    {
        Self::$writeData(Self::$getContainerDefinitionData($definition, isset($options['omit_tags']) && $options['omit_tags']), $options);
    }
    ...
    {
        Self::$writeData(Self::$getContainerAliasData($alias), $options);
    }
    ...
    {
        Self::$writeData(Self::$getEventDispatcherListenersData($eventDispatcher, array_key_exists('event', $options) ? $options['event'] : null), $options);
    }
    ...
    {
        Self::$writeData(Self::$getCallableData($callable, $options), $options);
    }
    ...
    protected function describeContainerParameter($parameter, array $options = array())
    {
        ...
        Self::$writeData(array($key => $parameter), $options);
    }
    ...
    private function writeData(array $data, array $options)
    {
        ...
        Self::$write(json_encode($data, $flags | JSON_PRETTY_PRINT)."\n");
    }
    ...
    private function getEventDispatcherListenersData(EventDispatcherInterface $eventDispatcher, $event = null)
    {
        ...
        if (null !== $event) {
            foreach ($registeredListeners as $listener) {
                $l = Self::$getCallableData($listener);
            ...
            foreach ($registeredListeners as $eventListened => $eventListeners) {
                foreach ($eventListeners as $eventListener) {
                    $l = Self::$getCallableData($eventListener);
