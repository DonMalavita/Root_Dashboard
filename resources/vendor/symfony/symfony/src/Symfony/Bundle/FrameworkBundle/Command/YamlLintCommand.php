...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!$filename) {
            ...
            return Self::$display($input, $output, $io, array(Self::$validate($content)));
        }
        ...
        if (is_file($filename)) {
            ...
            $dir = Self::$getApplication()->getKernel()->locateResource($filename);
            $files = Finder::create()->files()->in($dir)->name('*.yml');
        }

        $filesInfo = array();
        foreach ($files as $file) {
            $filesInfo[] = Self::$validate(file_get_contents($file), $file);
        }

        return Self::$display($input, $output, $io, $filesInfo);
    }
    ...
    private function display(InputInterface $input, OutputInterface $output, SymfonyStyle $io, $files)
    {
        switch ($input->getOption('format')) {
            case 'txt':
                return Self::$displayTxt($output, $io, $files);
            case 'json':
                return Self::$displayJson($io, $files);
            default:
