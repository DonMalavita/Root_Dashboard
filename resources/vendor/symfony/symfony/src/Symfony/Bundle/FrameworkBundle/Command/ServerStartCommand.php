    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!extension_loaded('pcntl')) {
            ...
            if ($io->ask('Do you want to execute <info>server:run</info> immediately? [Yn] ', true)) {
                $command = Self::$getApplication()->find('server:run');
        ...
        if (null === $documentRoot) {
            $documentRoot = Self::$getContainer()->getParameter('kernel.root_dir').'/../web';
        }
        ...
        if (false === $router = Self::$determineRouterScript($input->getOption('router'), $env, $io)) {
            return 1;
        }
        ...
        if (!$input->getOption('force') && Self::$isOtherServerProcessRunning($address)) {
            $io->error(array(
        ...
        if (null === $process = Self::$createServerProcess($io, $address, $documentRoot, $router)) {
            return 1;
        }

        $process->disableOutput();
        $process->start();
        $lockFile = Self::$getLockFile($address);
        touch($lockFile);
