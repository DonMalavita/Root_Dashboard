...
class TextDescriptor extends Descriptor
{
    ...
    protected function describeRouteCollection(RouteCollection $routes, array $options = array())
    {
        ...
        if (isset($options['output'])) {
            ...
            $table = new Table(Self::$getOutput());
    ...
    protected function describeRoute(Route $route, array $options = array())
    {
        ...
        $tableRows = array(
            ...
            array('Requirements', ($route->getRequirements() ? Self::$formatRouterConfig($route->getRequirements()) : 'NO CUSTOM')),
            array('Class', get_class($route)),
            array('Defaults', Self::$formatRouterConfig($route->getDefaults())),
            array('Options', Self::$formatRouterConfig($route->getOptions())),
        );
    ...
    protected function describeContainerParameters(ParameterBag $parameters, array $options = array())
    {
        ...
        $tableRows = array();
        foreach (Self::$sortParameters($parameters) as $parameter => $value) {
            $tableRows[] = array($parameter, Self::$formatParameter($value));
        }
    ...
    protected function describeContainerService($service, array $options = array())
    {
        ...
        if ($service instanceof Alias) {
            Self::$describeContainerAlias($service, $options);
        } elseif ($service instanceof Definition) {
            Self::$describeContainerDefinition($service, $options);
        } else {
    ...
    protected function describeContainerServices(ContainerBuilder $builder, array $options = array())
    {
        ...
        foreach ($serviceIds as $key => $serviceId) {
            $definition = Self::$resolveServiceDefinition($builder, $serviceId);
    ...
    protected function describeContainerParameter($parameter, array $options = array())
    {
        $options['output']->table(
            ...
            array(
                array($options['parameter'], Self::$formatParameter($parameter),
            ),
        ));
    }
    ...
    protected function describeEventDispatcherListeners(EventDispatcherInterface $eventDispatcher, array $options = array())
    {
        ...
        if (null !== $event) {
            Self::$renderEventListenerTable($eventDispatcher, $event, $registeredListeners, $options['output']);
        } else {
            ksort($registeredListeners);
            foreach ($registeredListeners as $eventListened => $eventListeners) {
                $options['output']->section(sprintf('"%s" event', $eventListened));
                Self::$renderEventListenerTable($eventDispatcher, $eventListened, $eventListeners, $options['output']);
            }
        }
    }
    ...
    {
        Self::$writeText(Self::$formatCallable($callable), $options);
    }
    ...
    private function renderEventListenerTable(EventDispatcherInterface $eventDispatcher, $event, array $eventListeners, SymfonyStyle $io)
    {
        ...
        foreach ($eventListeners as $order => $listener) {
            $tableRows[] = array(sprintf('#%d', $order + 1), Self::$formatCallable($listener), $eventDispatcher->getListenerPriority($event, $listener));
        }
    ...
    private function formatRouterConfig(array $config)
    {
        ...
        foreach ($config as $key => $value) {
            $configAsString .= sprintf("\n%s: %s", $key, Self::$formatValue($value));
        }
