...
class SecurityExtension extends Extension
{
    ...
    {
        foreach (Self::$listenerPositions as $position) {
            Self::$factories[$position] = array();
        }
    }
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $mainConfig = Self::$getConfiguration($configs, $container);

        $config = Self::$processConfiguration($mainConfig, $configs);
        ...
        Self::$createFirewalls($config, $container);
        Self::$createAuthorization($config, $container);
        Self::$createRoleHierarchy($config, $container);

        if ($config['encoders']) {
            Self::$createEncoders($config['encoders'], $container);
        }

        // load ACL
        if (isset($config['acl'])) {
            Self::$aclLoad($config['acl'], $container);
        }

        // add some required classes for compilation
        Self::$addClassesToCompile(array(
    ...
    private function aclLoad($config, ContainerBuilder $container)
    {
        ...
        Self::$configureDbalAclProvider($config, $container, $loader);
    }
    ...
    private function createAuthorization($config, ContainerBuilder $container)
    {
        ...
        foreach ($config['access_control'] as $access) {
            $matcher = Self::$createRequestMatcher(
                $container,
                $access['path'],
                $access['host'],
                $access['methods'],
            ...
            if ($access['allow_if']) {
                $attributes[] = Self::$createExpression($container, $access['allow_if']);
            }
    ...
    private function createFirewalls($config, ContainerBuilder $container)
    {
        ...
        $providerIds = Self::$createUserProviders($config, $container);
        ...
        foreach ($firewalls as $name => $firewall) {
            list($matcher, $listeners, $exceptionListener) = Self::$createFirewall($container, $name, $firewall, $authenticationProviders, $providerIds);
    ...
    private function createFirewall(ContainerBuilder $container, $id, $firewall, &$authenticationProviders, $providerIds)
    {
        ...
        if (isset($firewall['request_matcher'])) {
            ...
            $matcher = Self::$createRequestMatcher($container, $pattern, $host, $methods);
        }
        ...
        if (isset($firewall['provider'])) {
            $defaultProvider = Self::$getUserProviderId($firewall['provider']);
        } else {
        ...
        if (false === $firewall['stateless']) {
            ...
            $listeners[] = new Reference(Self::$createContextListener($container, $contextKey));
        }
        ...
        list($authListeners, $defaultEntryPoint) = Self::$createAuthenticationListeners($container, $id, $firewall, $authenticationProviders, $defaultProvider, $configuredEntryPoint);

        $listeners = array_merge($listeners, $authListeners);

        // Switch user listener
        if (isset($firewall['switch_user'])) {
            $listeners[] = new Reference(Self::$createSwitchUserListener($container, $id, $firewall['switch_user'], $defaultProvider));
        }
        ...
        // Exception listener
        $exceptionListener = new Reference(Self::$createExceptionListener($container, $firewall, $id, $configuredEntryPoint ?: $defaultEntryPoint, $firewall['stateless']));
    ...
    {
        if (isset(Self::$contextListeners[$contextKey])) {
            return Self::$contextListeners[$contextKey];
        }

        $listenerId = 'security.context_listener.'.count(Self::$contextListeners);
        $listener = $container->setDefinition($listenerId, new DefinitionDecorator('security.context_listener'));
        $listener->replaceArgument(2, $contextKey);

        return Self::$contextListeners[$contextKey] = $listenerId;
    }
    ...
    private function createAuthenticationListeners($container, $id, $firewall, &$authenticationProviders, $defaultProvider, $defaultEntryPoint)
    {
        ...
        foreach (Self::$listenerPositions as $position) {
            foreach (Self::$factories[$position] as $factory) {
                $key = str_replace('-', '_', $factory->getKey());

                if (isset($firewall[$key])) {
                    $userProvider = isset($firewall[$key]['provider']) ? Self::$getUserProviderId($firewall[$key]['provider']) : $defaultProvider;
    ...
    private function createEncoders($encoders, ContainerBuilder $container)
    {
        ...
        foreach ($encoders as $class => $encoder) {
            $encoderMap[$class] = Self::$createEncoder($encoder, $container);
        }
    ...
    private function createUserProviders($config, ContainerBuilder $container)
    {
        ...
        foreach ($config['providers'] as $name => $provider) {
            $id = Self::$createUserDaoProvider($name, $provider, $container);
            $providerIds[] = $id;
        }
    ...
    {
        $name = Self::$getUserProviderId(strtolower($name));

        // Doctrine Entity and In-memory DAO provider are managed by factories
        foreach (Self::$userProviderFactories as $factory) {
            $key = str_replace('-', '_', $factory->getKey());
        ...
        if (isset($provider['chain'])) {
            ...
            foreach ($provider['chain']['providers'] as $providerName) {
                $providers[] = new Reference(Self::$getUserProviderId(strtolower($providerName)));
            }
    ...
    {
        $userProvider = isset($config['provider']) ? Self::$getUserProviderId($config['provider']) : $defaultProvider;
    ...
    {
        if (isset(Self::$expressions[$id = 'security.expression.'.sha1($expression)])) {
            return Self::$expressions[$id];
        }
        ...
        $container
            ...
            ->addArgument(serialize(Self::$getExpressionLanguage()->parse($expression, array('token', 'user', 'object', 'roles', 'request', 'trust_resolver'))->getNodes()))
        ;

        return Self::$expressions[$id] = new Reference($id);
    }
    ...
    private function createRequestMatcher($container, $path = null, $host = null, $methods = array(), $ip = null, array $attributes = array())
    {
        ...
        if (isset(Self::$requestMatchers[$id])) {
            return Self::$requestMatchers[$id];
        }
    ...
    public function getConfiguration(array $config, ContainerBuilder $container)
    {
        ...
        return new MainConfiguration(Self::$factories, Self::$userProviderFactories);
    }
