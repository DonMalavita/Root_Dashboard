...
abstract class AbstractFactory implements SecurityFactoryInterface
{
    ...
    public function create(ContainerBuilder $container, $id, $config, $userProviderId, $defaultEntryPointId)
    {
        ...
        $authProviderId = Self::$createAuthProvider($container, $id, $config, $userProviderId);

        // authentication listener
        $listenerId = Self::$createListener($container, $id, $config, $userProviderId);

        // add remember-me aware tag if requested
        if (Self::$isRememberMeAware($config)) {
            $container
        ...
        $entryPointId = Self::$createEntryPoint($container, $id, $config, $defaultEntryPointId);
    ...
    public function addConfiguration(NodeDefinition $node)
    {
        ...
        foreach (array_merge(Self::$options, Self::$defaultSuccessHandlerOptions, Self::$defaultFailureHandlerOptions) as $name => $default) {
            if (is_bool($default)) {
    ...
    protected function createListener($container, $id, $config, $userProvider)
    {
        ...
        $listener->replaceArgument(5, new Reference(Self::$createAuthenticationSuccessHandler($container, $id, $config)));
        $listener->replaceArgument(6, new Reference(Self::$createAuthenticationFailureHandler($container, $id, $config)));
        $listener->replaceArgument(7, array_intersect_key($config, Self::$options));
    ...
    {
        $successHandlerId = Self::$getSuccessHandlerId($id);
        $options = array_intersect_key($config, Self::$defaultSuccessHandlerOptions);
    ...
    {
        $id = Self::$getFailureHandlerId($id);
        $options = array_intersect_key($config, Self::$defaultFailureHandlerOptions);
    ...
    {
        return 'security.authentication.success_handler.'.$id.'.'.str_replace('-', '_', Self::$getKey());
    }

    protected function getFailureHandlerId($id)
    {
        return 'security.authentication.failure_handler.'.$id.'.'.str_replace('-', '_', Self::$getKey());
    }
}
