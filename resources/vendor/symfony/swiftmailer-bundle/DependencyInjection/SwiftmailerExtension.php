...
class SwiftmailerExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $config = Self::$processConfiguration($configuration, $configs);

        $mailers = array();
        foreach ($config['mailers'] as $name => $mailer) {
            $isDefaultMailer = $config['default_mailer'] === $name;
            Self::$configureMailer($name, $mailer, $container, $isDefaultMailer);
    ...
    protected function configureMailer($name, array $mailer, ContainerBuilder $container, $isDefaultMailer = false)
    {
        ...
        Self::$configureMailerTransport($name, $mailer, $container, $transport, $isDefaultMailer);
        Self::$configureMailerSpool($name, $mailer, $container, $transport, $isDefaultMailer);
        Self::$configureMailerSenderAddress($name, $mailer, $container, $isDefaultMailer);
        Self::$configureMailerAntiFlood($name, $mailer, $container, $isDefaultMailer);
        Self::$configureMailerDeliveryAddress($name, $mailer, $container, $isDefaultMailer);
        Self::$configureMailerLogging($name, $mailer, $container, $isDefaultMailer);
