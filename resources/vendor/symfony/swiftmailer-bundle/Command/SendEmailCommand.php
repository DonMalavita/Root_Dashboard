...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($name) {
            Self::$processMailer($name, $input, $output);
        } else {
            $mailers = array_keys(Self::$getContainer()->getParameter('swiftmailer.mailers'));
    ...
    {
        if (!Self::$getContainer()->has(sprintf('swiftmailer.mailer.%s', $name))) {
        ...
        if (Self::$getContainer()->getParameter(sprintf('swiftmailer.mailer.%s.spool.enabled', $name))) {
            $mailer = Self::$getContainer()->get(sprintf('swiftmailer.mailer.%s', $name));
            ...
            if ($transport instanceof \Swift_Transport_SpoolTransport) {
                ...
                $sent = $spool->flushQueue(Self::$getContainer()->get(sprintf('swiftmailer.mailer.%s.transport.real', $name)));
