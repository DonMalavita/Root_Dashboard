...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($name) {
            Self::$outputMailer($output, $name);
        } else {
            Self::$outputMailers($output);
        }
    }

    protected function outputMailers(OutputInterface $output, $routes = null)
    {
        $output->writeln(Self::$getHelper('formatter')->formatSection('swiftmailer', 'Current mailers'));
        ...
        $mailers = Self::$getContainer()->getParameter('swiftmailer.mailers');
        foreach ($mailers as $name => $mailer) {
            $mailer = Self::$getContainer()->get($mailer);
            $transport = Self::$getContainer()->getParameter(sprintf('swiftmailer.mailer.%s.transport.name', $name));
            $spool = Self::$getContainer()->getParameter(sprintf('swiftmailer.mailer.%s.spool.enabled', $name)) ? 'YES' : 'NO';
            $delivery = Self::$getContainer()->getParameter(sprintf('swiftmailer.mailer.%s.delivery.enabled', $name)) ? 'YES' : 'NO';
            $singleAddress = Self::$getContainer()->getParameter(sprintf('swiftmailer.mailer.%s.single_address', $name));

            if (Self::$isDefaultMailer($name)) {
    ...
    protected function outputMailer(OutputInterface $output, $name)
    {
        try {
            ...
            $mailer = Self::$getContainer()->get($service);
        ...
        $output->writeln(Self::$getHelper('formatter')->formatSection('swiftmailer', sprintf('Mailer "%s"', $name)));
        if (Self::$isDefaultMailer($name)) {
        ...
        if (Self::$getContainer()->hasParameter(sprintf('swiftmailer.spool.%s.file.path', $name))) {
            $output->writeln(sprintf('<comment>Spool file</comment>     %s', Self::$getContainer()->getParameter(sprintf('swiftmailer.spool.%s.file.path', $name))));
        }
    ...
    {
        return (Self::$getContainer()->getParameter('swiftmailer.default_mailer') == $name || 'default' == $name) ? true : false;
    }
}
